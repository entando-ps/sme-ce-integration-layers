...
select * from tabnucleifull where rif_istanza = (select idIstanza from tabistanza
                                                 where id_sponsor=(select idSoggetto from tabsoggetto where codiceFiscale='GDFGMN70D16H501T') and rif_tipoIstanza in (4, 5) ORDER BY idIstanza DESC LIMIT 1);
...
INSERT INTO cartaesercito.tabcartaesercito (rif_soggetto, numeroCarta, dataRilascioCarta,
                                            ...
                                            controllo)
VALUES ((select idSoggetto from tabsoggetto where codiceFiscale in ('GDFGMN70D16H501T')), 'numeroCarta', '2022-09-28', '2022-09-28', 1, 1, 'motivazioneRevoca', 1, '2022-09-28 16:09:12',
        'dataProduzioneCarta', 1, 'dataEmissionePvc', 'dataRevocaPvc', (select tabnucleifull.id from tabnucleifull join tabsoggetto on tabnucleifull.rif_soggetto=tabsoggetto.idSoggetto where tabsoggetto.codiceFiscale in ('GDFGMN70D16H501T')), '1', 1, 1, 1, 1, 'dataStampaPVC',
        'dataInvioPvc', 1);

delete from tabcartaesercito;
...
from tabcartaesercito
         ...
         join tabistanza on tabnucleifull.rif_istanza = tabistanza.idIstanza
        where tabistanza.idIstanza = (select idIstanza from tabistanza
                                      where id_sponsor=(select idSoggetto from tabsoggetto where codiceFiscale='GDFGMN70D16H501T') and rif_tipoIstanza in (1, 3) ORDER BY idIstanza DESC LIMIT 1);
